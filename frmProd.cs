...
namespace MainMenu
{
    public partial class frmProd : Form
    {
        ...
        string ProcCode;
        string EmpCode;
        static double T_Cones = 0;
        static double T_CLU = 0;
        ...
        private void FormatGrid()
        {
            ...
            gv.Columns[4].Width = 80;
            gv.Columns[5].Width = 50;
            gv.Columns[6].Width = 50;
            gv.Columns[7].Width = 67;
            gv.Columns[0].HeaderText = "DRUGLINE";
            gv.Columns[1].HeaderText = "ARTICLE";
            gv.Columns[2].HeaderText = "LENGTH";
            gv.Columns[3].HeaderText = "W.SPINDLE";
            gv.Columns[4].HeaderText = "SHADE";
            gv.Columns[5].HeaderText = "CONES";
            gv.Columns[6].HeaderText = "CLU";
            gv.Columns[7].HeaderText = "TIME(HR)";
            gv.Columns[7].DefaultCellStyle.Format = "0.0#";


            gv.Columns[2].ReadOnly = true;
            gv.Columns[3].ReadOnly = true;
            gv.Columns[6].ReadOnly = true;
            gv.Columns[7].ReadOnly = true;
        ...
        {
            string sql = "SELECT P_Code+'  '+P_Descrip as PName FROM PRD_PROCESS where CATEGORY='F' AND P_CODE NOT IN ('006','007','008') ORDER BY P_CODE";
        ...
        {
            string sql = "SELECT Art_No, P_Code from Article WHERE P_code='" + ProcCode + "' and Category='F' order by P_code";
            SqlCommand cmd = new SqlCommand(sql, con);
            con.Open();
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet();
            ...
            comboBox1.ValueMember = "Article.Art_No";
            con.Close();
        }
...
//                    comProc.Focus();
        ...
        private void comEmpID_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                con.Open();
                String sql1 = "SELECT S.DL,S.ART_NO,A.LENGTH,  M.W_SP, S.SHADE, S.PART_PROD AS CONES , A.LENGTH/10000*S.PART_PROD  AS CLU, M.MACHINE FROM PRODMAST M " +
                ...
                if (dt1.Rows.Count > 0)
                {
                        ...
                        MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                    {
                        clear();
                        FormatGrid();
                        comboBox1.Text = dt1.Rows[0].Field<string>("ART_NO");
                        ...
                        for (int i = 0; i < dt1.Rows.Count; i++)
                        {
                            ...
                            (gv.Rows[i].Cells[1]).Value = dt1.Rows[i].Field<string>("ART_NO"); ;
                            (gv.Rows[i].Cells[2]).Value = dt1.Rows[i].Field<double>("LENGTH"); ;
                            (gv.Rows[i].Cells[3]).Value = dt1.Rows[i].Field<double>("W_SP"); ;
                            (gv.Rows[i].Cells[4]).Value = dt1.Rows[i].Field<string>("SHADE"); ;
                            (gv.Rows[i].Cells[5]).Value = dt1.Rows[i].Field<double>("CONES"); ;
                            (gv.Rows[i].Cells[6]).Value = dt1.Rows[i].Field<double>("CLU"); ;
        ...
        private void comboBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                con.Open();
                String sql = "SELECT * FROM Article WHERE Art_No = '" + comboBox1.Text + "' and P_Code='" + ProcCode + "' ";
                SqlDataAdapter sda = new SqlDataAdapter(sql, con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                con.Close();
                if (dt.Rows.Count.ToString() == "1")
                {
                    ...
                    comboBox1.Visible = false;
                    (gv.Rows[row].Cells[2]).Value = dt.Rows[0].Field<double>("Length");
                    (gv.Rows[row].Cells[3]).Value = dt.Rows[0].Field<double>("Spindle");

                    gv.Focus();
                    gv.Rows[row].Cells[4].Selected = true;
                    gv.CurrentCell = gv[4, row];
                    gv.BeginEdit(true);
...
//                    if (gv[5, row].Value != null && gv[5, row].Value.ToString() != String.Empty)
                    {
                        if (ConeEdit == false)
                        {
                            cal();
        ...
        private void save()
        {
            ...
            for (int i = 0; i < gv.Rows.Count; i++)
            {
                ...
                if (gv[5, i].Value != null && gv[5, i].Value.ToString() != String.Empty)
                {
                    ...
                    if (gv[4, i].Value != null && gv[4, i].Value.ToString() != String.Empty)
                    {
                        arrayItem[k, 2] = ((gv.Rows[i].Cells[4]).Value).ToString();
                ...
                else
                {
                    ...
                    {
                        cone = cone + Convert.ToDouble(((gv.Rows[i].Cells[5]).Value).ToString());
                        time = time + Convert.ToDouble(((gv.Rows[i].Cells[7]).Value).ToString());
                    }
                    WSP = Convert.ToDouble(((gv.Rows[i].Cells[3]).Value).ToString());
            ...
            for (int x = 0; x <= aLen; x++)
            {
                ...
                if (Convert.ToDouble(arrayArt[x, 1]) != 0)
                {
                    String sql = "INSERT INTO PRODMAST (ENO,DATE,SHIFT, P_CODE, CATEGORY, ART_NO,  TOTAL_PROD,  W_TIME, W_SP, MACHINE,LOC) " +
                                 "values('" + EmpCode + "','" + dtProd.Value.Date + "','" + txtShift.Text + "', '" + ProcCode + "', '" + 'F' + "'," +
        ...
        private void cal()
        {
            ...
            for (int i = 0; i < gv.Rows.Count; i++)
            {
                ...
                if (gv[5, i].Value != null && Convert.ToDouble(gv[5, i].Value) != 0 && gv[5, i].Value.ToString() != String.Empty)
                {
                    ...
                    LEN = Convert.ToDouble(((gv.Rows[i].Cells[2]).Value).ToString());
                    Cones = Convert.ToDouble(((gv.Rows[i].Cells[5]).Value).ToString());
                    CLU = LEN / 10000 * Cones;

                    T_Cones = T_Cones + Cones;
                    T_CLU = T_CLU + CLU;
                    W_HR = (CLU / T_CLU) * Convert.ToDouble(txtTime.Text);

                    (gv.Rows[i].Cells[6]).Value = CLU;
                    for (int k = 0; k < gv.Rows.Count; k++)
                    {
                        if (gv[5, k].Value != null && Convert.ToDouble(gv[5, k].Value) != 0 && gv[5, k].Value.ToString() != String.Empty)
                        {
                            (gv.Rows[k].Cells[7]).Value = ((Convert.ToDouble(((gv.Rows[k].Cells[2]).Value).ToString())) / 10000 * (Convert.ToDouble(((gv.Rows[k].Cells[5]).Value).ToString()))) / T_CLU * Convert.ToDouble(txtTime.Text);
                        }
                    }

                }
                txtTot.Text = T_Cones.ToString();
                txtCLU.Text = T_CLU.ToString();
        ...
        {
            if (gv.CurrentCell.ColumnIndex == 4)
            {
        ...
        private void frmProd_KeyPress(object sender, KeyPressEventArgs e)
        {
