...
namespace MainMenu
{
    public partial class frmMakeUpProd : Form
    {


        public frmMakeUpProd()
        {
            InitializeComponent();
        ...
        static string[,] arrayArt = new string[25, 4];
        static string[,] arrayItem = new string[25, 4];
        ...
        {
            string sql = "SELECT P_Code+'  '+P_Descrip as PName FROM PRD_PROCESS where CATEGORY='P'";
        ...
        private void comEmpID_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                String sql1 = "SELECT S.DL,S.ART_NO AS COUNT,  M.W_SP, S.SHADE, S.PART_PROD AS KG , M.MACHINE FROM PRODMAST M " +
                              "LEFT JOIN PRODSUB S ON M.ENO =S.ENO AND M.DATE=S.DATE AND M.SHIFT=S.SHIFT AND m.ART_NO=s.ART_NO " +
                              "LEFT JOIN ARTICLE A ON M.ART_NO=A.ART_NO AND M.P_CODE=A.P_CODE " +
                              "WHERE M.ENO ='" + EmpCode + "' and M.DATE= '" + dtProd.Value.Date + "' and M.SHIFT= '" + txtShift.Text + "' and M.P_CODE= '" + ProcCode + "'  ";
                //                "WHERE M.ENO ='" + EmpCode + "' and M.DATE= '" + Convert.ToDateTime(dtProd.Text).ToString("dd-MM-yyyy") + "' and M.SHIFT= '" + txtShift.Text + "' and M.P_CODE= '" + ProcCode + "'  ";
        ...
        private void save()
        {
            ...
            con.Open();
            String sqlM = "SELECT * FROM PRODMAST WHERE ENO ='" + EmpCode + "' and Date= '" + dtProd.Value.Date + "' and shift= '" + txtShift.Text + "' and P_code= '" + ProcCode + "'   ";
            SqlDataAdapter sdaM = new SqlDataAdapter(sqlM, con);
            DataTable dtM = new DataTable();
            sdaM.Fill(dtM);
            if (dtM.Rows.Count > 0)
            {
                String sql2 = "DELETE FROM PRODMAST WHERE ENO ='" + EmpCode + "' and Date= '" + dtProd.Value.Date + "' and shift= '" + txtShift.Text + "' and P_code= '" + ProcCode + "'   ";
                //                String sql2 = "DELETE FROM PRODMAST WHERE ENO ='" + EmpCode + "' and Date= '" + Convert.ToDateTime(dtProd.Text).ToString("dd-MM-yyyy") + "' and shift= '" + txtShift.Text + "' and P_code= '" + ProcCode + "'   ";
            ...
            for (int x = 0; x <= aLen; x++)
            {
                ...
                {
                    String sql = "INSERT INTO PRODMAST (ENO,DATE,SHIFT, P_CODE, CATEGORY, ART_NO,  TOTAL_PROD,  W_TIME, W_SP, MACHINE) " +
                                 "values('" + EmpCode + "','" + dtProd.Value.Date + "','" + txtShift.Text + "', '" + ProcCode + "', '" + 'P' + "'," +
                                 " '" + arrayArt[x, 0] + "', '" + Convert.ToDouble(arrayArt[x, 1]) + "','" + Convert.ToDouble(arrayArt[x, 2]) + "','" + Convert.ToDouble(arrayArt[x, 3]) + "', '" + txtMach.Text + "')";
            ...
            String sqlS = "SELECT * FROM PRODSUB WHERE ENO ='" + EmpCode + "' and Date= '" + dtProd.Value.Date + "' and shift= '" + txtShift.Text + "' and P_code= '" + ProcCode + "'   ";
            ...
            if (dtS.Rows.Count > 0)
            {
                ...
                String sql3 = "DELETE FROM PRODSUB WHERE ENO ='" + EmpCode + "' and Date= '" + dtProd.Value.Date + "' and shift= '" + txtShift.Text + "' and P_code= '" + ProcCode + "'   ";
            ...
            for (int i = 0; i < gv.Rows.Count; i++)
            {
                ...
                {
                    String sql2 = "INSERT INTO PRODSUB (ENO,DATE,SHIFT, P_CODE,MACHINE, DL,  ART_NO , SHADE,PART_PROD) " +
                                 "values('" + EmpCode + "','" + dtProd.Value.Date + "','" + txtShift.Text + "', '" + ProcCode + "','" + txtMach.Text + "'," +
                                 " '" + arrayItem[i, 0] + "', '" + arrayItem[i, 1] + "','" + arrayItem[i, 2] + "','" + Convert.ToDouble(arrayItem[i, 3]) + "')";
